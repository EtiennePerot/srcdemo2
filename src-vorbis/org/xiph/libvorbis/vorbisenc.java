...
/*********************************************************************
 * Encoding using a VBR quality mode. The usable range is -.1 (lowest quality, smallest file) to 1. (highest quality, largest
 * file). Example quality mode .4: 44kHz stereo coupled, roughly 128kbps VBR ret = vorbis_encode_init_vbr(&vi,2,44100,.4);
 * --------------------------------------------------------------------- Encoding using an average bitrate mode (ABR). example:
 * 44kHz stereo coupled, average 128kbps VBR ret = vorbis_encode_init(&vi,2,44100,-1,128000,-1);
 * --------------------------------------------------------------------- Encode using a quality mode, but select that quality
 * mode by asking for an approximate bitrate. This is not ABR, it is true VBR, but selected using the bitrate interface, and
 * then turning bitrate management off: ret = ( vorbis_encode_setup_managed(&vi,2,44100,-1,128000,-1) ||
 * vorbis_encode_ctl(&vi,OV_ECTL_RATEMANAGE_AVG,NULL) || vorbis_encode_setup_init(&vi));
 *********************************************************************/
public class vorbisenc
{
	...
	public vorbisenc()
	{
		final setup_44 ve_setup_44_stereo = new org.xiph.libvorbis.modes.setup_44();
		setup_list = new ve_setup_data_template[] { ve_setup_44_stereo.data };
	}

	private int book_dup_or_new(final codec_setup_info ci, final static_codebook book)
	{
		int i;
	...
	private boolean get_setup_template(final int ch, final int srate, float req, final int q_or_bitrate)
	{
		int i = 0, j;
		final highlevel_encode_setup hi = vi.codec_setup.hi;
		if (q_or_bitrate > 0) {
			req /= ch;
		}
		while (setup_list[i] != null) {
			if (setup_list[i].coupling_restriction == -1 || setup_list[i].coupling_restriction == ch) {
				if (srate >= setup_list[i].samplerate_min_restriction && srate <= setup_list[i].samplerate_max_restriction) {
					final int mappings = setup_list[i].mappings;
					float[] map;
					if (q_or_bitrate > 0) {
					...
					if (j == mappings) {
						...
						final float low = map[j];
						final float high = map[j + 1];
						final float del = (req - low) / (high - low);
	...
	{
		final codec_setup_info ci = vi.codec_setup;
		final highlevel_encode_setup hi = ci.hi;
		final ve_setup_data_template setup = hi.setup;
		final int is = new Float(hi.base_setting).intValue();
		final float ds = hi.base_setting - is;
		final int ch = vi.channels;
		final float[] r = setup.rate_mapping;
		if (r == null) {
			return (-1);
		}
	...
	private void vorbis_encode_ath_setup(final int block)
	{
		final codec_setup_info ci = vi.codec_setup;
		final vorbis_info_psy p = ci.psy_param[block];
		p.ath_adjatt = ci.hi.ath_floating_dB;
		p.ath_maxatt = ci.hi.ath_absolute_dB;
	}

	private void vorbis_encode_blocksize_setup(final float s, final int[] shortb, final int[] longb)
	{
		final codec_setup_info ci = vi.codec_setup;
		final int is = new Float(s).intValue();
	...
	private void vorbis_encode_compand_setup(final float s, final int block, final compandblock[] in, final float[] x)
	{
		int i;
	...
	public Object vorbis_encode_ctl(final vorbis_info _vi, final int number, final Object arg)
	{
		if (_vi == null) {
			return false;
		}
		...
		switch (number) {
			// now deprecated *****************
			/*
			 * case OV_ECTL_RATEMANAGE_GET: { ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg;
			 * ai.management_active=hi.managed;
			 * ai.bitrate_hard_window=ai.bitrate_av_window=(double)hi.bitrate_reservoir/vi.rate; ai.bitrate_av_window_center=1.;
			 * ai.bitrate_hard_min=hi.bitrate_min; ai.bitrate_hard_max=hi.bitrate_max; ai.bitrate_av_lo=hi.bitrate_av;
			 * ai.bitrate_av_hi=hi.bitrate_av; } return true; // now deprecated ***************** case OV_ECTL_RATEMANAGE_SET: {
			 * ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg; if ( ai == null ) { hi.managed=0; } else {
			 * hi.managed=ai.management_active; vorbis_encode_ctl( vi, OV_ECTL_RATEMANAGE_AVG, arg ); vorbis_encode_ctl( vi,
			 * OV_ECTL_RATEMANAGE_HARD, arg ); } } return true; // now deprecated ***************** case OV_ECTL_RATEMANAGE_AVG:
			 * { ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg; if (ai == null) { hi.bitrate_av=0; } else {
			 * hi.bitrate_av=(ai.bitrate_av_lo+ai.bitrate_av_hi)*.5; } } return true; // now deprecated ***************** case
			 * OV_ECTL_RATEMANAGE_HARD: { ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg; if (ai == null ) {
			 * hi.bitrate_min=0; hi.bitrate_max=0; } else { hi.bitrate_min=ai.bitrate_hard_min;
			 * hi.bitrate_max=ai.bitrate_hard_max;
			 * hi.bitrate_reservoir=ai.bitrate_hard_window*(hi.bitrate_max+hi.bitrate_min)*.5; } if ( hi.bitrate_reservoir <
			 * 128.f ) hi.bitrate_reservoir=128; } return true;
			 */
			// replacement ratemanage interface
			case OV_ECTL_RATEMANAGE2_GET: {
				final ovectl_ratemanage2_arg ai = (ovectl_ratemanage2_arg) arg;
				if (ai == null) {
					return false;
				}
				ai.management_active = hi.managed;
	...
	private void vorbis_encode_floor_setup(final float s, final int block, final static_codebook[][] books,
			final vorbis_info_floor1[] in, final int[] x)
	{
		int i, k;
		final int is = new Float(s).intValue();
		final codec_setup_info ci = vi.codec_setup;
		...
		final vorbis_info_floor1 f = new vorbis_info_floor1(in[x[is]]);
	...
	private void vorbis_encode_global_psych_setup(final float s, final vorbis_info_psy_global[] in, final float[] x)
	{
		int i;
		int is = new Float(s).intValue();
		float ds = s - is;
		...
		final vorbis_info_psy_global g = ci.psy_g_param;
		ds = x[is] * (1.0f - ds) + x[is + 1] * ds;
		is = new Float(ds).intValue();
		ds -= is;
		if (ds == 0 && is > 0) {
		...
		for (i = 0; i < 4; i++) {
			g.preecho_thresh[i] = new Double(in[is].preecho_thresh[i] * (1. - ds) + in[is + 1].preecho_thresh[i] * ds)
					.floatValue();
			g.postecho_thresh[i] = new Double(in[is].postecho_thresh[i] * (1. - ds) + in[is + 1].postecho_thresh[i] * ds)
					.floatValue();
		}
		g.ampmax_att_per_sec = ci.hi.amplitude_track_dBpersec;
	}

	private void vorbis_encode_global_stereo(final highlevel_encode_setup hi, final adj_stereo[] p)
	{
		final float s = hi.stereo_point_setting;
		int i;
		final int is = new Float(s).intValue();
		final float ds = s - is;
		final codec_setup_info ci = vi.codec_setup;
		...
		if (p != null) {
			...
			if (hi.managed != 0) {
				...
				float kHz = new Double(p[is].kHz[PACKETBLOBS / 2] * (1. - ds) + p[is + 1].kHz[PACKETBLOBS / 2] * ds)
						.floatValue();
				for (i = 0; i < PACKETBLOBS; i++) {
					g.coupling_pointlimit[0][i] = new Double(kHz * 1000. / vi.rate * ci.blocksizes[0]).intValue();
					g.coupling_pointlimit[1][i] = new Double(kHz * 1000. / vi.rate * ci.blocksizes[1]).intValue();
				...
				kHz = new Double(p[is].lowpasskHz[PACKETBLOBS / 2] * (1. - ds) + p[is + 1].lowpasskHz[PACKETBLOBS / 2] * ds)
						.floatValue();
				for (i = 0; i < PACKETBLOBS; i++) {
					g.sliding_lowpass[0][i] = new Double(kHz * 1000. / vi.rate * ci.blocksizes[0]).intValue();
					g.sliding_lowpass[1][i] = new Double(kHz * 1000. / vi.rate * ci.blocksizes[1]).intValue();
	...
	public boolean vorbis_encode_init(final vorbis_info _vi, final int channels, final int rate, final int max_bitrate,
			final int nominal_bitrate, final int min_bitrate)
	{
		if (_vi == null) {
			return false;
		}
	...
	public boolean vorbis_encode_init_vbr(final vorbis_info _vi, final int channels, final int rate, final float base_quality)
	{
		if (_vi == null) {
	...
	private void vorbis_encode_map_n_res_setup(final float s, final vorbis_mapping_template[] maps)
	{
		final codec_setup_info ci = vi.codec_setup;
		int i, j;
		final int is = new Float(s).intValue();
		int modes = 2;
		final vorbis_info_mapping0[] map = maps[is].map;
		final vorbis_info_mode[] mode = { new vorbis_info_mode(0, 0, 0, 0), new vorbis_info_mode(1, 0, 0, 1) }; // _mode_template
		final vorbis_residue_template[] res = maps[is].res;
	...
	private void vorbis_encode_noisebias_setup(final float s, final int block, final int[] suppress, final noise3[] in,
			final noiseguard[] guard, final float userbias)
	{
		int i, j;
		final int is = new Float(s).intValue();
		final float ds = s - is;
		...
		for (j = 0; j < P_NOISECURVES; j++) {
			final float min = p.noiseoff[j][0] + 6; // the lowest it can go
			for (i = 0; i < P_BANDS; i++) {
	...
	private void vorbis_encode_peak_setup(final float s, final int block, final int[] suppress)
	{
		final int is = new Float(s).intValue();
		final float ds = s - is;
		final codec_setup_info ci = vi.codec_setup;
	...
	private void vorbis_encode_psyset_setup(final float s, final int[] nn_start, final int[] nn_partition,
			final float[] nn_thresh, final int block)
	{
		final codec_setup_info ci = vi.codec_setup;
		final highlevel_encode_setup hi = ci.hi;
		final int is = new Float(s).intValue();
		...
		ci.psy_param[block] = new vorbis_info_psy(
				...
				1, -0.f, .5f, .5f, 0, 0, 0, new float[][] { { -1 }, { -1 }, { -1 } }, new float[] { -1 }, 105.f, 0, 0, -1, -1,
				0.0f);
		final vorbis_info_psy p = ci.psy_param[block];
		p.blockflag = block >> 1;
	...
	private void vorbis_encode_residue_setup(final int number, final int block, final vorbis_residue_template res)
	{
		final codec_setup_info ci = vi.codec_setup;
		int i;
		...
		final vorbis_info_residue0 r = ci.residue_param[number];
		...
		// fill in all the books
		{
			...
			if (ci.hi.managed != 0) {
				...
				for (i = 0; i < r.partitions; i++) {
					for (k = 0; k < 3; k++) {
						if (res.books_base_managed.books[i][k] != null) {
							final int bookid = book_dup_or_new(ci, res.books_base_managed.books[i][k]);
				...
				for (i = 0; i < r.partitions; i++) {
					for (k = 0; k < 3; k++) {
						if (res.books_base.books[i][k] != null) {
							final int bookid = book_dup_or_new(ci, res.books_base.books[i][k]);
							r.booklist[booklist++] = bookid;
		...
		// lowpass setup/pointlimit
		{
			...
			final vorbis_info_floor1 f = ci.floor_param[block]; // by convention
			final float nyq = vi.rate / 2.0f;
			...
			if (ci.residue_type[block] == 2) {
				r.end = new Double((freq / nyq * blocksize * 2) / r.grouping + .9).intValue() * r.grouping; // round up only if
																											// we're well past
			}
			else {
				r.end = new Double((freq / nyq * blocksize) / r.grouping + .9).intValue() * r.grouping; // round up only if
																										// we're well past
			}
		}
	}
	...
	{ // the final setup call
		...
		vorbis_encode_psyset_setup(hi.short_setting, setup.psy_noise_normal_start[0], setup.psy_noise_normal_partition[0],
				setup.psy_noise_normal_thresh, 0);
		vorbis_encode_psyset_setup(hi.short_setting, setup.psy_noise_normal_start[0], setup.psy_noise_normal_partition[0],
				setup.psy_noise_normal_thresh, 1);
		if (singleblock == 0) {
			vorbis_encode_psyset_setup(hi.long_setting, setup.psy_noise_normal_start[1], setup.psy_noise_normal_partition[1],
					setup.psy_noise_normal_thresh, 2);
			vorbis_encode_psyset_setup(hi.long_setting, setup.psy_noise_normal_start[1], setup.psy_noise_normal_partition[1],
					setup.psy_noise_normal_thresh, 3);
		}
		// tone masking setup
		vorbis_encode_tonemask_setup(hi.block[i0].tone_mask_setting, 0, setup.psy_tone_masteratt, setup.psy_tone_0dB,
				setup.psy_tone_adj_impulse);
		vorbis_encode_tonemask_setup(hi.block[1].tone_mask_setting, 1, setup.psy_tone_masteratt, setup.psy_tone_0dB,
				setup.psy_tone_adj_other);
		if (singleblock == 0) {
			vorbis_encode_tonemask_setup(hi.block[2].tone_mask_setting, 2, setup.psy_tone_masteratt, setup.psy_tone_0dB,
					setup.psy_tone_adj_other);
			vorbis_encode_tonemask_setup(hi.block[3].tone_mask_setting, 3, setup.psy_tone_masteratt, setup.psy_tone_0dB,
					setup.psy_tone_adj_long);
		}
		// noise companding setup
		vorbis_encode_compand_setup(hi.block[i0].noise_compand_setting, 0, setup.psy_noise_compand,
				setup.psy_noise_compand_short_mapping);
		vorbis_encode_compand_setup(hi.block[1].noise_compand_setting, 1, setup.psy_noise_compand,
				setup.psy_noise_compand_short_mapping);
		if (singleblock == 0) {
			vorbis_encode_compand_setup(hi.block[2].noise_compand_setting, 2, setup.psy_noise_compand,
					setup.psy_noise_compand_long_mapping);
			vorbis_encode_compand_setup(hi.block[3].noise_compand_setting, 3, setup.psy_noise_compand,
					setup.psy_noise_compand_long_mapping);
		}
		// peak guarding setup
		...
		vorbis_encode_noisebias_setup(hi.block[i0].noise_bias_setting, 0, setup.psy_noise_dBsuppress,
				setup.psy_noise_bias_impulse, setup.psy_noiseguards, userbias);
		vorbis_encode_noisebias_setup(hi.block[1].noise_bias_setting, 1, setup.psy_noise_dBsuppress,
				setup.psy_noise_bias_padding, setup.psy_noiseguards, 0.0f);
		if (singleblock == 0) {
			vorbis_encode_noisebias_setup(hi.block[2].noise_bias_setting, 2, setup.psy_noise_dBsuppress,
					setup.psy_noise_bias_trans, setup.psy_noiseguards, 0.0f);
			vorbis_encode_noisebias_setup(hi.block[3].noise_bias_setting, 3, setup.psy_noise_dBsuppress,
					setup.psy_noise_bias_long, setup.psy_noiseguards, 0.0f);
		}
		...
		if (hi.managed > 0) {
			ci.bi = new bitrate_manager_info(hi.bitrate_av, hi.bitrate_min, hi.bitrate_max, hi.bitrate_reservoir,
					hi.bitrate_reservoir_bias, hi.bitrate_av_damp);
		}
		return true;
	}

	public boolean vorbis_encode_setup_managed(final vorbis_info _vi, final int channels, final int rate,
			final int max_bitrate, int nominal_bitrate, final int min_bitrate)
	{
		vi = _vi;
		final float tnominal = new Integer(nominal_bitrate).floatValue();
		...
		if (!get_setup_template(channels, rate, nominal_bitrate, 1)) {
			System.out.println("Unable to find setup template - " + channels + " channels, " + rate + " rate, "
	...
	private boolean vorbis_encode_setup_setting(final int channels, final int rate)
	{
		int i, is;
		final codec_setup_info ci = vi.codec_setup;
		final highlevel_encode_setup hi = ci.hi;
	...
	public boolean vorbis_encode_setup_vbr(final int channels, final int rate, float quality)
	{
		quality += .0000001;
		if (quality >= 1.) {
			quality = .9999f;
		...
		if (!get_setup_template(channels, rate, quality, 0)) {
			System.out.println("Unable to find setup template - " + channels + " channels, " + rate + " rate, " + quality
					+ " quality");
			return false;
		}
	...
	private void vorbis_encode_tonemask_setup(final float s, final int block, final att3[] att, final int[] max,
			final vp_adjblock[] in)
	{
		int i;
		final int is = new Float(s).intValue();
		final float ds = s - is;
