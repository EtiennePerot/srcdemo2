...
public class vorbis_block
{
	...
	private float[][] _vp_quantize_couple_memo(vorbis_info_psy_global g, vorbis_look_psy p, vorbis_info_mapping0 vi,
			float[][] mdct)
	{
		int i, j;
		int n = p.n;
	...
	static void sortindex_fix8(int[] index, int ioff, float[] data, int offset)
	{
		...
		k = 4;
		for (i = 0; i < 8; i++)
			index[ioff + offset + i] = n[((k >= 8) || (j < 4)
					&& (Math.abs(data[offset + 0 + n[j]]) >= Math.abs(data[offset + 0 + n[k]])) ? j++ : k++)]
					+ offset;
	}
	...
	static void sortindex_fix32(int[] index, int ioff, float[] data, int offset)
	{
		...
		for (i = j = 0, k = 8; i < 16; i++)
			n[i] = index[ioff
					+ ((k >= 16) || (j < 8) && (Math.abs(data[0 + index[ioff + j]]) >= Math.abs(data[0 + index[ioff + k]])) ? j++
							: k++)];
		for (i = j = 16, k = 24; i < 32; i++)
			n[i] = index[ioff
					+ ((k >= 32) || (j < 24) && (Math.abs(data[0 + index[ioff + j]]) >= Math.abs(data[0 + index[ioff + k]])) ? j++
							: k++)];
		...
		if (p.vi.normal_point_p > 0) {
			...
			for (i = 0; i < vi.coupling_steps; i++) {
				...
				for (j = 0; j < n; j += partition) {
					/*
					 * for ( k=0; k < partition; k++ ) { // work[k] = mags[i]+k+j; work[k] = new float[ mags[i].length - k+j ];
					 * System.arraycopy(mags[i], k+j, work[k], 0, mags[i].length - k+j ); } //
					 * qsort(work,partition,sizeof(*work),apsort); Arrays.sort( work ); // for( k=0; k < partition; k++ ) //
					 * ret[i][k+j] = work[k]-mags[i]; for ( k=0; k < partition; k++) ret[i][k+j] = work[k] - mags[i];
					 */
	...
	-0.009935f, -0.011245f, -0.012726f, -0.014397f, -0.016282f, -0.018407f, -0.020800f, -0.023494f, -0.026522f, -0.029923f,
			-0.033737f, -0.038010f, -0.042787f, -0.048121f, -0.054064f, -0.060671f, -0.068000f, -0.076109f, -0.085054f,
			-0.094892f, -0.105675f, -0.117451f, -0.130260f, -0.144134f, -0.159093f, -0.175146f, -0.192286f, -0.210490f,
	...
	private void _vp_couple(int blobno, vorbis_info_psy_global g, vorbis_look_psy p, vorbis_info_mapping0 vi, float[][] res,
			float[][] mag_memo, int[][] mag_sort, int[][] ifloor, int[] nonzero, int sliding_lowpass)
	{
		int i, j, k;
		int n = p.n;
					...
					for (k = 0; k < partition; k++) {
						...
						if (l < sliding_lowpass) {
							if ((l >= limit && Math.abs(rM[l]) < postpoint && Math.abs(rA[l]) < postpoint)
								...
								float[] return_buffer = precomputed_couple_point(mag_memo[i][l], floorM[l], floorA[l], rM[qM
										+ l], rA[qA + l]);
	...
	private boolean res2_forward(oggpack_buffer opb_local, vorbis_look_residue0 vl, float[][] in, int in_offset, float[][] out,
			int[] nonzero, int ch, int[][] partword)
	{
		int i, j, k; // long
		int n = pcmend / 2; // long
		int used = 0; // long
	...
	public void mapping0_forward()
	{
		...
		for (i = 0; i < vi.channels; i++) {
			...
			// causes flr errors later call to accumulate_fit( logmask, logmdct, look.sorted_index[i], look.sorted_index[i+1],
			// fits[i], n, info );
		...
		for (i = 0; i < vi.channels; i++) {
			...
			// b.psy[ psy_look_offset ]._vp_noisemask( logmdct, noise); // noise does not have by-frequency offset bias applied
			// yet
			b.psy[psy_look_offset]._vp_noisemask(logfft, logmdct, noise); // noise does not have by-frequency offset bias
																			// applied yet
			...
			if (vorbis_bitrate_managed() && (floor_posts[i][PACKETBLOBS / 2] != null)) {
				...
				for (k = 1; k < PACKETBLOBS / 2; k++) {
					floor_posts[i][k] = floor1_interpolate_fit(b.flr[info.floorsubmap[submap]], floor_posts[i][0],
				...
				for (k = PACKETBLOBS / 2 + 1; k < PACKETBLOBS - 1; k++) {
					floor_posts[i][k] = floor1_interpolate_fit(b.flr[info.floorsubmap[submap]],
							floor_posts[i][PACKETBLOBS / 2], floor_posts[i][PACKETBLOBS - 1], (k - PACKETBLOBS / 2) * 65536
									/ (PACKETBLOBS / 2));
				}
			}
		}
		...
		for (k = (vorbis_bitrate_managed() ? 0 : PACKETBLOBS / 2); k <= (vorbis_bitrate_managed() ? PACKETBLOBS - 1
			...
			if (info.coupling_steps > 0) {
				_vp_couple(k, ci.psy_g_param, b.psy[psy_look_offset], info, pcm, mag_memo, mag_sort, ilogmaskch, nonzero,
			...
			for (i = 0; i < info.submaps; i++) {
				...
				// classifications = _residue_P[ci.residue_type[resnum]].class( vb, b.residue[resnum], couple_bundle,
				...
				// _residue_P[ci.residue_type[resnum]].forward( vb, b.residue[resnum], couple_bundle, NULL, zerobundle,
				// ch_in_bundle, classifications );
				res2_forward(opb_local, b.residue[resnum], couple_bundle, couple_bundle_offset, null, zerobundle, ch_in_bundle,
						classifications);
			}
