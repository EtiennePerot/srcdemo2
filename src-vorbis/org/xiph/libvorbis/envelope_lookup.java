...
class envelope_lookup
{
	...
	public envelope_lookup(int _ch, int _winlength, int _searchstep, float _minenergy, mdct_lookup _mdct, float[] _mdct_win,
			envelope_band[] _band, envelope_filter_state[] _filter, int _stretch, int[] _mark, int _storage, int _current,
	...
	{
		this(src.ch, src.winlength, src.searchstep, src.minenergy, src.mdct, src.mdct_win, src.band, src.filter, src.stretch,
				src.mark, src.storage, src.current, src.curmark, src.cursor);
	}
	...
	public int _ve_amp(vorbis_info_psy_global gi, float[] data, int offset1, envelope_band[] bands,
			envelope_filter_state[] filters, int offset2, int pos)
	{
		int n = winlength;
		int ret = 0;
		int i, j;
		float decay;
		/*
		 * we want to have a 'minimum bar' for energy, else we're just basing blocks on quantization noise that outweighs the
		 * signal itself (for low power signals)
		 */
		...
		/*
		 * perform spreading and limiting, also smooth the spectrum. yes, the MDCT results in all real coefficients, but it
		 * still *behaves* like real/imaginary pairs
		 */
		for (i = 0; i < n / 2; i += 2) {
