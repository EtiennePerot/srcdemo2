...
public class ogg_stream_state
{
	...
	long[] granule_vals; // ogg_int64_t //granulepos values for headers. Not compact this way, but it is simple coupled to the
							// lacing fifo
	...
	int serialno; // long
	int pageno; // long
	long packetno; // ogg_int64_t // sequence number for decode; the framing knows where there's a hole in the data, but we need
					// coupling so
	...
	public boolean ogg_stream_packetin(ogg_packet op)
	{
		...
		if (body_returned > 0) {
			/*
			 * advance packet data according to the body_returned pointer. We had to keep it around to return a pointer into the
			 * buffer last call
			 */
		...
		/*
		 * Copy in the submitted packet. Yes, the copy is a waste; this is the liability of overly clean abstraction for the
		 * time being. It will actually be fairly easy to eliminate the extra copy in the future
		 */
	...
	/*
	 * This will flush remaining packets into a page (returning nonzero), even if there is not enough data to trigger a flush
	 * normally (undersized page). If there are no packets or partial packets to flush, ogg_stream_flush returns 0. Note that
	 * ogg_stream_flush will try to flush a normal sized page like ogg_stream_pageout; a call to ogg_stream_flush does not
	 * guarantee that all packets have flushed. Only a return value of 0 from ogg_stream_flush indicates all packet data is
	 * flushed into pages. since ogg_stream_flush will flush the last page in a stream even if it's undersized, you almost
	 * certainly want to use ogg_stream_pageout (and *not* ogg_stream_flush) unless you specifically need to flush an page
	...
	public boolean ogg_stream_flush(ogg_page og)
	{
		...
		if (pageno == -1)
			pageno = 0;
		/*
		 * because someone called stream_reset; this would be a strange thing to do in an encode stream, but it has plausible
		 * uses
		 */
		// local OOP varname serialno to serialno_local
