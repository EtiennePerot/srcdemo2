...
/**
 * This class defines the configuration options that may not change throughout a FLAC stream. In general, these setting must be
 * set to match the input audio used(sample rate, sample size, channels, etc). After a stream has started, these settings must
 * not change.
 * 
...
public class StreamConfiguration implements Cloneable
{
	...
	int sampleRate;
	/*
	 * is the currently set configuration valid? Encoders should not attempt to use an invalid configuration
	 */
	boolean validConfig = false;

	/**
	 * Constructor, sets defaults for most values. Some default values must be changed to match the audio
	 * characteristics(channel count, sample rate, sample size).
	 */
	...
	/**
	 * Constructor, allows setting of all options. In general, parameters given must match the input audio characteristics.
	 * minBlock and maxBlock may be set as desired, though minBlock is expected to be less than or equal to maxBlock. If
	 * minBlock or maxBlock is out of a valid range, it will be automatically adjusted to the closest valid value.
	 * 
	 * @param channelCount
	 ...
	 */
	public StreamConfiguration(final int channelCount, final int minBlock, final int maxBlock, final int sampleRate,
	 ...
	 */
	public StreamConfiguration(final StreamConfiguration sc)
	{
	...
	/**
	 * Test if this StreamConfiguration and a paired EncodingConfiguration define a Subset compliant stream. FLAC defines a
	 * subset of options to ensure resulting FLAC streams are streamable.
	 * 
	 * @param ec
	 *            EncodingConfiguration object to check against
	 * @return true if these configurations are Subset compliant, false otherwise.
	 */
	public boolean isEncodingSubsetCompliant(final EncodingConfiguration ec)
	{
		boolean result = true;
	...
	/**
	 * Test if stream is Subset compliant. FLAC defines a subset of options to ensure resulting FLAC streams are streamable. Not
	 * all options in that subset are defined by the StreamConfiguration class, however, but exist in the EncodingConfiguration
	 * class. Therefore, the alternative method
	 * {@link StreamConfiguration#isEncodingSubsetCompliant(javaFlacEncoder.EncodingConfiguration) isEncodingSubsetCompliant}
	 * should be checked as well to ensure the combined Stream/Encoding configurations are BOTH valid.
	 * 
	...
	/**
	 * Test if the current configuration is valid. While most set methods will ensure the values are in a valid range before
	 * setting them, some settings(such as number of channels), cannot be guessed. This method may alter current values to make
	 * them valid if possible.
	 * 
	...
	/**
	 * Set the bits per sample. Because this is not a value that may be guessed and corrected, the value will be set to that
	 * given even if it is not valid.
	 * 
	 * @param bitsPerSample
	 *            number of bits per sample
	 * @return true if value given is within the valid range, false otherwise.
	 */
	public boolean setBitsPerSample(final int bitsPerSample)
	{
		final boolean result = ((bitsPerSample <= MAX_BITS_PER_SAMPLE) && (bitsPerSample >= MIN_BITS_PER_SAMPLE));
		this.bitsPerSample = bitsPerSample;
		return result;
	}

	/**
	 * Set number of channels in stream. Because this is not a value that may be guessed and corrected, the value will be set to
	 * that given even if it is not valid.
	 * 
	 * @param count
	 *            Number of channels
	 * @return true if the channel count is within the valid range, false otherwise.
	 */
	public boolean setChannelCount(final int count)
	{
		final boolean result = count > 0 && count <= MAX_CHANNEL_COUNT;
		channelCount = count;
		return result;
	}

	/**
	 * Set the maximum block size to use. If this value is out of a valid range, it will be set to the closest valid value. User
	 * must ensure that this value is set above or equal to the minimum block size.
	 * 
	 * @param size
	 *            maximum block size to use.
	 * @return actual size set
	 */
	public int setMaxBlockSize(final int size)
	{
	...
	/**
	 * Set the minimum block size to use. If this value is out of a valid range, it will be set to the closest valid value. User
	 * must ensure that this value is set below or equal to the maximum block size.
	 * 
	 * @param size
	 *            minimum block size to use.
	 * @return actual size set
	 */
	public int setMinBlockSize(final int size)
	{
	...
	/**
	 * Set the sample rate. Because this is not a value that may be guessed and corrected, the value will be set to that given
	 ...
	 */
	public boolean setSampleRate(final int rate)
	{
		final boolean result = (rate <= MAX_SAMPLE_RATE && rate >= MIN_SAMPLE_RATE);
		sampleRate = rate;
		return result;
	}
}
