...
/**
 * MetadataBlockStreamInfo is used to declare the initial stream parameters, such as Sample Rate, bits per sample, and number of
 * channels, as well as information on the encoded stream such as total number of samples, minimum and maximum block and frame
 * sizes, and md5sum of raw audio samples. A StreamInfo block must be the first meta-data block in a FLAC stream, and only one
...
public class MetadataBlockStreamInfo
{
	...
	/**
	 * Create a FLAC StreamInfo metadata block with the given parameters. Because of the data stored in a StreamInfo block, this
	 * should generally be created only after all encoding is done.
	 * 
	 * @param sc
	 ...
	 */
	public static EncodedElement getStreamInfo(final StreamConfiguration sc, final int minFrameSize, final int maxFrameSize,
			final long samplesInStream, final byte[] md5Hash)
	{
		final EncodedElement ele = new EncodedElement();
		final int bytes = getByteSize();
		final byte data[] = new byte[bytes];
		ele.setData(data);
		ele.setUsableBits(bytes * 8);
		final int encodedBitsPerSample = sc.getBitsPerSample() - 1;
	...
	/*
	 * int minimumBlockSize = 4096;//32768;//16 bits used; 16 minimum valid int maximumBlockSize = 4096;//32768;//16 bits used;
	 * 65535 maximum valid int minimumFrameSize = 0;//24 bits used; zero implies unknown int maximumFrameSize = 0;//24 bits
	 * used; zero implies unknown int sampleRate = 8000;//4096;//20 bits used, but 655350Hz max. 0 is invalid. byte
	 * numberOfChannels = 1;//3 bits used byte bitsPerSample = 16;//5 bits used. values 4-32 valid long totalSamplesInStream =
	 * 0;//36 bits used. 0 implies unknown. byte[] md5Hash;
	 */
	/**
