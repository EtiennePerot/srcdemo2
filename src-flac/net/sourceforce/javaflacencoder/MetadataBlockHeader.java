...
/**
 * The MetadataBlockHeader class is used to creat FLAC compliant Metadata Block Headers. See the FLAC specification for more
 * information.
 * 
...
public class MetadataBlockHeader
{
	...
	public enum MetadataBlockType
	{
		...
		CUESHEET,
		/**
		 * A meta-block to pad the stream, allowing other meta-data to be written in the future without re-writing the entire
		 * stream.
		 */
		PADDING,
	...
	/**
	 * Create a meta-data block header of the given type, and return the result in a new EncodedElement(so data is ready to be
	 * placed directly in FLAC stream)
	 * 
	 * @param lastBlock
	 ...
	 */
	public static EncodedElement getMetadataBlockHeader(final boolean lastBlock, final MetadataBlockType type, final int length)
	{
		final EncodedElement ele = new EncodedElement();
		final byte[] data = new byte[4];
		final int encodedLastBlock = (lastBlock) ? 1 : 0;
		int index = 0;
		EncodedElement.addInt(encodedLastBlock, 1, index, data);
		index++;
		int encodedType = 0;
		final MetadataBlockType[] vals = MetadataBlockType.values();
