...
/**
 * FLAC_ConsoleFileEncoder is the command line interface to this FLAC encoder. It accepts several flags and parameters to alter
...
public class FLAC_ConsoleFileEncoder
{
	...
	public static String _MAX_BLOCK = "-bmax";
	/** flag to specify minimum block size. An integer must follow */
	public static String _MIN_BLOCK = "-bmin";
	/** flag to display help */
	public static String HELP = "-h";
	/** Error message given for incorrect maximum block size specified */
	public static final String MAX_BLOCK_ERROR = "Error with maximum block " + "size: integer value between "
			+ StreamConfiguration.MIN_BLOCK_SIZE + " & " + StreamConfiguration.MAX_BLOCK_SIZE + " must follow \"" + _MIN_BLOCK
			+ "\"";
	...
	public static final String MIN_BLOCK_ERROR = "Error with minimum block " + "size: integer value between "
			+ StreamConfiguration.MIN_BLOCK_SIZE + " & " + StreamConfiguration.MAX_BLOCK_SIZE + " must follow \"" + _MIN_BLOCK
			+ "\"";
	...
	public static void main(final String[] args)
	{
		...
		final FLAC_ConsoleFileEncoder cfe = new FLAC_ConsoleFileEncoder(args);
	...
	/**
	 * Constructor. Uses given arguments to immediately attempts to encode the file given. Uses stdout and stderr for notices of
	 * progress and error.
	 * 
	 * @param args
	 *            Encoding arguments to use.
	 */
	public FLAC_ConsoleFileEncoder(final String[] args)
	{
		outputFile = null;
		inputFile = null;
		// create configurations/handle input arguments and set configurations
		final boolean valid = handleArguments(args);
		if (attemptEncode) {
			printSmallCopyright();
		}
		// create encoder with configurations
		final FLAC_FileEncoder enc = new FLAC_FileEncoder();
	...
	{
		final File fIn = new File("input.wav");
		final File fOut = new File("fileEncoderTest.flac");
		final FLAC_FileEncoder enc = new FLAC_FileEncoder();
		enc.encode(fIn, fOut);
	}

	private boolean getInputFile(final String filename)
	{
		boolean result = true;
	 ...
	 */
	int getInt(final String[] args, final int index)
	{
		int result = -1;
	...
	private boolean getOutputFile(final String filename)
	{
		boolean result = true;
		final File file = new File(filename);
		/*
	...
	private boolean handleArguments(final String[] args)
	{
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals(_MIN_BLOCK)) {
			...
			else if (args[i].equals(_MAX_BLOCK)) {
				...
				i++;
				final String type = (i < args.length) ? args[i] : "";
	...
	{
		final String copyrightNotice = "Copyright (C) 2010  Preston Lacey\n" + "All Rights Reserved.\n\n"
	...
	{
		final String message = "javaFlacEncoder Copyright (C) 2010  Preston Lacey\n"
