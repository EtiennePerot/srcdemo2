...
/**
 * This class defines the configuration options that are allowed to change within a FLAC stream. Options here may be changed
 * from one frame to the next. In general, the settings should not need altered, but the option to do so remains.
 * 
...
{
	/**
	 * Defines the options for channel configuration to use. LEFT & RIGHT channels refers to stereo audio as expected.
	 * INDEPENDENT channels refer to each channel encoded separately. SIDE channel is the difference of the LEFT and RIGHT
	 * channels(LEFT-RIGHT). MID channel is the integer average of LEFT and RIGHT channels( (LEFT+RIGHT)/2 ). Options using MID
	 * and/or SIDE channels may benefit encoding by taking advantage of similarities between the channels, and are available in
	 * FLAC for STEREO streams only. In general, ENCODER_CHOICE should be chosen.
	 */
	...
	/**
	 * Defines the various subframe types that may be used. If you don't know what these are, choose EXHAUSTIVE(for description
	 * of subframe types, see the flac format documentation at http://flac.sourceforge.net/format.html)
	 */
	 ...
	 */
	public EncodingConfiguration(final EncodingConfiguration e)
	{
	...
	/**
	 * Set the channel configuration to use. Channel configuration refers to the way multiple channels are processed. See
	 * documentation for {@link ChannelConfig ChannelConfig} for more info on choices.
	 * 
	 * @param conf
	 *            Channel configuration to use.
	 */
	public void setChannelConfig(final ChannelConfig conf)
	{
		channelConfig = conf;
	}
	 ...
	 * {@link EncodingConfiguration#MAX_LPC_ORDER MAX_LPC_ORDER} and {@link EncodingConfiguration#MIN_LPC_ORDER MIN_LPC_ORDER}),
	 * it will be set to the closest valid value instead.
	 * 
	 * @param order
	 *            maximum LPC order to use
	 */
	public void setMaxLPCOrder(final int order)
	{
	 ...
	 */
	public void setMinLPCOrder(final int order)
	{
	...
	/**
	 * Set the subframe type to use. This refers to the way each subframe(channel) is compressed. See documentation for
	 * {@link SubframeType SubframeType} for more info on choices.
	 * 
	 * @param type
	 */
	public void setSubframeType(final SubframeType type)
	{
		subframeType = type;
	}
}
