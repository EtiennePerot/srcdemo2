...
public class Subframe_LPC extends Subframe
{
	...
	private static int getParam(final int[] vals, final int end, final int start, final int max)
	{
		long sum = 0;
		...
		final float mean = (float) sum / (float) (end - start);
	...
	private static int partialEncodeLPC(final int[] samples, final int count, final int start, final int increment,
			final LPC lpc, final Subframe_LPC lpcSubframe, final int frameSampleSize)
	{
		// System.err.println("encodeLPC begin");
		final int order = lpc.order;
		...
		final int headerSize = order * frameSampleSize + precision * order + 9 + 8;
		final int[] coeffs = lpcSubframe._quantizedCoeffs;
		final int shift = quantizeCoefficients(lpc.rawCoefficients, coeffs, order, precision);
		...
		final int coeff1 = coeffs[1];
		final int coeff2 = coeffs[2];
		final int coeff3 = coeffs[3];
		final int coeff4 = coeffs[4];
		final int coeff5 = coeffs[5];
		final int coeff6 = coeffs[6];
		final int coeff7 = coeffs[7];
		final int coeff8 = coeffs[8];
		final int coeff9 = coeffs[9];
		final int coeff10 = coeffs[10];
		final int coeff11 = coeffs[11];
		final int coeff12 = coeffs[12];
		int baseIndex = start;
		...
		final int riceSize = RiceEncoder.calculateEncodeSize(lpcSubframe._errors, order, 1, count - order,
				(int) tempLowOrderBits);
		final int totalSize = headerSize + riceSize;
	 ...
	 * @return
	 */
	private static int quantizeCoefficients(final double[] coefficients, final int[] dest, final int order, final int precision)
	{
		int shiftApplied = 0;
		final int maxValAllowed = 1 << (precision - 1) - 1;
		double maxVal = 0;
		...
		for (shiftApplied = 15; shiftApplied > 0; shiftApplied--) {
			final int temp = (int) (maxVal * (1 << shiftApplied));
	...
	private static void writeHeadersAndData(final EncodedElement dataEle, final int order, final int[] coeff,
			final int precision, final int shift, final int[] samples, final int sampleSize, final int start, final int skip)
	{
		// write headers
		final int encodedType = 1 << 5 | (order - 1);
		dataEle.addInt(0, 1);
		...
		for (int i = 1; i <= order; i++) {
			final int val = -coeff[i];
			dataEle.addInt(val, precision);
		}
	}

	private static void writeLPC(final int[] samples, final int count, final int start, final int increment,
			final EncodedElement ele, final int frameSampleSize, final int riceParam, final int precision, final int shift,
			final int[] coeffs, final int[] errors, final int order, final RiceEncoder rice)
	{
		writeHeadersAndData(ele, order, coeffs, precision, shift, samples, frameSampleSize, start, increment - 1);
		final int paramSize = (riceParam > 14) ? 5 : 4;
		final boolean fiveBitParam = (paramSize < 5) ? false : true;
	...
	Subframe_LPC(final StreamConfiguration sc)
	{
		super(sc);
	...
	public int encodeSamples(final int[] samples, final int count, final int start, final int skip,
			final EncodedElement dataEle, final int offset, final int unencSampleSize)
	{
		encodeSamples(samples, count, start, skip, offset, unencSampleSize);
		final EncodedElement result = getData();
		final int totalBits = result.getTotalBits();
	...
	public int encodeSamples(final int[] samples, final int count, final int start, final int skip, final int offset,
			final int unencSampleSize)
	{
		final int encodedSamples = count;
		if (DEBUG_LEV > 0) {
		...
		final int minOrder = ec.getMinLPCOrder();
		final int maxOrder = ec.getMaxLPCOrder();
		final int frameSampleSize = unencSampleSize;
		int order = -1;
		int totalBits = 0;
		long[] R = correlations;
		...
		for (int i = maxOrder; i >= minOrder; i--) {
			...
			final int tempTotalBits = partialEncodeLPC(samples, count, start, increment, lpcs[i], this, frameSampleSize);
	...
	{
		final EncodedElement result = new EncodedElement(_totalBits / 8 + 1, _offset);
