...
public class Subframe_Verbatim extends Subframe
{
	 ...
	 */
	Subframe_Verbatim(final StreamConfiguration sc)
	{
		super(sc);
			...
			final int offset, final int bitsPerSample)
	{
		...
		final int bits = bitsPerSample * count + offset + 1 * 8;
		int bytesNeeded = bits / 8;
		if (bits % 8 != 0) {
			bytesNeeded++;
		}
		final byte[] dataArray = new byte[bytesNeeded];
	...
	public int estimateSize(final int count, final int bitsPerSample)
	{
		final int estimatedSize = 8 + count * bitsPerSample;// header size + unencoded data.
