...
public class Subframe_Fixed extends Subframe
{
	 ...
	 */
	public Subframe_Fixed(final StreamConfiguration sc)
	{
		super(sc);
			...
			final EncodedElement dataEle, final int offset, final int unencSampleSize)
	{
		...
		final int[] error1 = _error1;
		final int[] error2 = _error2;
		final int[] error3 = _error3;
		final int[] error4 = _error4;
		...
		// int lowOrderBits = 0;
		for (int i = 0; i < 5; i++) {
			final double tempLowOrderBits = Math.log(2) * (sum[i] / (count - i));
		...
		final byte data[] = new byte[bits[order] / 8 + 10];
		// byte data[] = new byte[154*count+100];
		int[] errors = null;
		final int errorCount = count - order;
		...
		// write headers
		final int encodedType = 1 << 3 | order;
		dataEle.addInt(0, 1);
		...
		final int paramSize = (lowOrderBits[order] > 14) ? 5 : 4;
		final boolean fiveBitParam = (paramSize < 5) ? false : true;
		RiceEncoder.beginResidual(fiveBitParam, (byte) 0, dataEle);
		/*
		 * for(int i = 0; i < errorCount; i++) { int error = errors[errorOffset+i*errorStep]; if(error >= 32767 || error <=
		 * -32767) System.err.println("Error Bound issue?: " + error); }
		 */
		rice.encodeRicePartition(errors, errorOffset, errorStep, errorCount, dataEle, lowOrderBits[order], fiveBitParam);
			...
			final int unencSampleSize)
	{
		...
		for (int i = 0; i < 5; i++) {
			...
			final double tempLowOrderBits = LOG_2 * (sumsX / (count - i));
	...
	{
		final EncodedElement dataEle = new EncodedElement(_totalBits / 8 + 1, _offset);
		final int unencSampleSize = _frameSampleSize;
		// write headers
		final int encodedType = 1 << 3 | _order;
		dataEle.addInt(0, 1);
		dataEle.addInt(encodedType, 6);
		dataEle.addInt(0, 1);
		if (_order > 0) {
		...
		// send best data to rice encoder
		final int paramSize = (lowOrderBits[_order] > 14) ? 5 : 4;
		final boolean fiveBitParam = (paramSize < 5) ? false : true;
		RiceEncoder.beginResidual(fiveBitParam, (byte) 0, dataEle);
		/*
		 * for(int i = 0; i < errorCount; i++) { int error = errors[errorOffset+i*errorStep]; if(error >= 32767 || error <=
		 * -32767) System.err.println("Error Bound issue?: " + error); }
		 */
		rice.encodeRicePartition(_errors, _errorOffset, _errorStep, _errorCount, dataEle, lowOrderBits[_order], fiveBitParam);
		lastEncodedSize = dataEle.getTotalBits();
		if (DEBUG_LEV > 0) {
			System.err.println("Subframe_Fixed::encodeSamples(...): End");
		}
	...
	/*
	 * private int[] calculatePartitionsCount(int[] errors, int errorOffset, int errorStep, int errorCount) { int maxPartitions
	 * = 8; int[][] sums = new int[(int)Math.pow(2,maxPartitions)][]; for(int i = 0; i < maxPartitions; i++) { sums[i] = new
	 * int[(int)Math.pow(2, i)]; } int[] counts = new int[maxPartitions]; int[] usedPartitions = new int[maxPartitions]; int[]
	 * lastPartitionCount = new int[maxPartitions]; for(int i = 0; i < maxPartitions; i++) { counts[i] =
	 * errorCount/sums[i].length; if(errorCount % sums[i].length != 0) counts[i]++; usedPartitions[i] = errorCount/counts[i];
	 * if(errorCount %counts[i] != 0) usedPartitions[i]++; } for(int i = 0; i < maxPartitions; i++) { int temp =
	 * errorCount+errorOffset/ } int[] sizes = new int[maxPartitions]; int temp = 0; double log2 = Math.log(2); for(int i = 0; i
	 * < errorCount; i++) { temp = errors[errorOffset+i*errorStep]; if(temp < 0) temp = -temp; for(int x = 0; x < maxPartitions;
	 * x++) { float destDiv = i/counts[x]; int destIndex = (int)(i/destDiv); sums[x][destIndex] += temp; } } //sum up all bit
	 * sizes per partition, choose best size. for(int i = 0; i < maxPartitions; i++) { int tempTotal = 0; sizes[i] = 0; for(int
	 * x = 0; x < usedPartitions[i]; x++) { tempTotal = (int)(Math.log(sums[i][x])/Math.log(2)); float destDiv =
	 * (float)errorCount/sums[x].length; int destCount = errorCount/sums[x].length; int destIndex = (int)(i/destDiv); sizes[i]
