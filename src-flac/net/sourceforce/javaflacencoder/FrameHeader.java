...
public class FrameHeader
{
	...
	private static final int definedBlockSizes[] = { -1, 192, 576, 1152, 2304, 4608, -1, -1, 256, 512, 1024, 2048, 4096, 8192,
			16384, 32768 };
	private static final int definedSampleRates[] = { 0, 88200, 176400, 192000, 8000, 16000, 22050, 24000, 32000, 44100, 48000,
			96000, -1, -1, -1, -1 };
	 ...
	 * @return
	 */
	private static byte encodeBlockSize(final int blockSize)
	{
		if (DEBUG_LEV > 0) {
	...
	private static byte encodeSampleRate(final int sampleRate)
	{
		if (DEBUG_LEV > 0) {
	...
	/**
	 * Constructor creates a new FrameHeader object which is ready to generate headers. We can't use static functions to do
	 * this, since the process uses a CRC8 object which must be instantiated.
	 */
	public FrameHeader()
	{
	...
	/**
	 * Create the header for a frame with the given parameters. Header data is stored out to an EncodedElement, in the proper
	 * form for a FLAC stream.
	 * 
	 * @param fixBlock
	 *            True to use a fixed block size, false to use variable. At this time, this *must* be set to True, as variable
	 ...
	 * @param channelAssign
	 *            Channel assignment used in this frame's encoding. See EncodingConfiguration class documentation for more
	 *            information.
	 * @param sampleSize
	 *            Bits per sample.
	 * @param frameNumber
	 *            For fixed block-size encodings, this is the frame-number starting at zero and incrementing by one. For
	 *            variable block encodings, this is the sample number of the first sample in the frame.
	 * @param channelCount
	 *            Number of channels in the stream.
	 * @return EncodedElement where the header is saved to.
	 */
	public EncodedElement createHeader(final boolean fixBlock, final int blockSize, final int sampleRate,
			final EncodingConfiguration.ChannelConfig channelAssign, final int sampleSize, final long frameNumber,
			final int channelCount, final EncodedElement result)
	{
		if (DEBUG_LEV > 0) {
		...
		final byte[] data = new byte[MAX_HEADER_SIZE];
		final int start = 0;
		int nextPos = start;
		// set blocking strategy bits
		final int blockingStrat = (fixBlock) ? 0 : 1;
		final byte[] encodedFrameNumber = UTF8Modified.convertToExtendedUTF8(frameNumber);
		// set block size bits
		final byte encodedBlockSize = encodeBlockSize(blockSize);
		if (encodedBlockSize == 0x6 || encodedBlockSize == 0x7) {
			useEndBlockSize = true;
		}
		// set sample rate bits
		final byte encodedSampleRate = encodeSampleRate(sampleRate);
		...
		nextPos += 1;
		for (final byte element : encodedFrameNumber) {
			EncodedElement.addLong(element, 8, nextPos, data);
			nextPos += 8;
		}
